package com.cloud.runball.module.match_football_association.widget.utils;
...
//            return false;
//        }
//        Character.UnicodeBlock unicodeBlock = Character.UnicodeBlock.of(text.charAt(0));
//        return  (unicodeBlock == Character.UnicodeBlock.CJK_COMPATIBILITY_IDEOGRAPHS
...
//            || unicodeBlock == Character.UnicodeBlock.GENERAL_PUNCTUATION);
    ...
    public String getSortKey(String sortKeyString) {
        String key = sortKeyString.trim().substring(0, 1).toUpperCase();
        if ("辽".equals(key)) {
            return "L";
        }
        if ("内".equals(key)) {
        ...
        if (key.matches("[A-Z]")) {
            ...
            String value = getPinYin(key).substring(0, 1).toUpperCase();
            return value;
        }
        return "#";
    }
