...
import com.cloud.runball.databinding.FragmentMainBinding;
...
//  private final String mExponent_title = "摇跑指数 = 第1分钟距离(米) / 半程马拉松耗时(分钟)";
  private final String mExponent_title = "活力指数 = 第1分钟距离x第1分钟最高转速";
//  private final String mExponent_title_en = "YPI = distance of 1 minute(m) / time of half marathon(min)";
  private final String mExponent_title_en = "ei = distance of 1 minute x highest rotation speed of 1st minute";
  ...
  protected void onContentView(@NonNull View view, @Nullable Bundle savedInstanceState) {
    ...
    tvNoticeIndexTip = binding.tvNoticeIndexTip;
    // Replace @OnClick with listeners
    View imgNoticeTip = view.findViewById(R.id.imgNoticeTip);
    if (imgNoticeTip != null) imgNoticeTip.setOnClickListener(this::onClick);
...
//    nodeInfoList.add(new NodeInfo(NodeConstant.TYPE_EXPONENT, mExponent_denominator, getString(R.string.lbl_match_tab_rank_1), "(" + mExponent_denominator / 1000 + "km)", "0.00", 0.6F));
    ...
    boolean isOpenNotice = (boolean) SPUtils.get(MainFragment.this.getContext(), "isOpenNotice", true);
    if (isOpenNotice) {
...
//      commonDialog = new AssociationCommonDialog(MainFragment.this.getContext());
...
//      float value = new BigDecimal(mMeter + "").multiply(new BigDecimal("60")).divide(new BigDecimal(mKeepPlayTime + ""), 2, BigDecimal.ROUND_HALF_UP).floatValue();
      ...
      String tipShow = isZhCn() ? exponent_denominator_tips_zh + " " + value : exponent_denominator_tips_en + " " + value;
      //距离提示
      ...
      mHandler.postDelayed(() -> {
        tvNoticeIndexTip.setText(String.format(getString(R.string.tv_run_notice_tip_cur), runball_exponent));
      }, 1);
    }
  }
  ...
  private void requestStartPlay() {
    ...
    if(AccountUtil.isUserAccount()) {
      ...
      playInfo.setSource(PlayingDataConstant.PlayingSource.RANKING);
...
//      }
      ...
      for (int i = 0; i < speedDetail.size(); i++) {
        ...
        newSpeedDetail.setSpeed(speedDetail.get(i));
        newSpeedDetail.setCircle(circleDetail.get(i));
...
//      }
  ...
  private void playStop(boolean isFinish, boolean isShowCard, boolean isExit) {
    ...
    if(AccountUtil.isUserAccount()) {
      if (loadupCommonDialog == null) {
        loadupCommonDialog = new AssociationCommonTipDialog(MainFragment.this.getContext());
    ...
    disposable.add(
        observable.subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread()).subscribeWith(new WristBallObserver<UserPlayModel>() {
          ...
          public void onError(int code, String msg) {
            ...
            AssociationCommonDialog loadupErrorDialog = new AssociationCommonDialog(MainFragment.this.getContext());
  ...
  private void uploadLocalMatchPlayV3(PlayInfo data, boolean isFinish, boolean isShowCard, boolean isExit) {
    ...
    List<Integer> circleDetail = new ArrayList<>();
    for (int i = 0; i < speedDetailData.size(); i++) {
      ...
      circleDetail.add(itemSpeedDetail.getCircle());
    }
    Integer[] speedDetailArr = speedDetail.toArray(new Integer[0]);
    Integer[] circleDetailArr = circleDetail.toArray(new Integer[0]);
      ...
      case ServiceNoticeConstant.CODE_NOTIFY_RUNNING: {
        ...
        if (isFirst) {
          if (pleaseStopCommonDialog == null) {
            pleaseStopCommonDialog = new AssociationCommonTipDialog(MainFragment.this.getContext());
...
//        mKeepPlayTime = runningTime;
      ...
      case ServiceNoticeConstant.CODE_NOTIFY_RUN_FINISH: {
        ...
        if (pleaseStopCommonDialog == null && !AssociationShareCardDialog.isShowing()) {
          stopedCommonDialog = new AssociationCommonDialog(MainFragment.this.getContext());
...
//  }
  ...
  public void onClick(View v) {
    if (v.getId() == R.id.imgNoticeTip) {
      if (!TextUtils.isEmpty(mExponent_title)) {
        PopupWindowIndex.self().build(getContext(), isZhCn() ? mExponent_title : mExponent_title_en).show(v);
      }
      ...
      boolean isOpenNotice = (boolean) SPUtils.get(MainFragment.this.getContext(), "isOpenNotice", true);
      if (isOpenNotice) {
        SPUtils.put(MainFragment.this.getContext(), "isOpenNotice", false);
        ivNoticeSwitch.setImageResource(R.mipmap.icon_notice_stop);
      } else {
        SPUtils.put(MainFragment.this.getContext(), "isOpenNotice", true);
        ivNoticeSwitch.setImageResource(R.mipmap.icon_notice);
      }
    }
  }
  ...
  private void showShareTargetDialog(Bitmap bitmap, boolean isEnd) {
    ...
    dialog.show(MainFragment.this.getContext(), new ShareTargetDialog.ConfirmCallBack() {
      @Override
      ...
      public void onShareTarget(ShareTargetDialog.ShareTarget shareTarget) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
          if (ActivityCompat.checkSelfPermission(MainFragment.this.requireContext(), Manifest.permission.WRITE_EXTERNAL_STORAGE) != PackageManager.PERMISSION_GRANTED) {
            ActivityCompat.requestPermissions(MainFragment.this.requireActivity(), new String[]{Manifest.permission.WRITE_EXTERNAL_STORAGE}, 188);
            return;
          }
        }
        ShareManage shareManage = new ShareManage();
        shareManage.shareBitmap(MainFragment.this.getActivity(), shareTarget.getType(), bitmap, new ShareManage.ShareCallback() {
          @Override
          public void onStart() {
  ...
  private void showShareCardDialog(boolean isEnd) {
    ...
    AssociationShareCardDialog.show(
        ...
        new AssociationShareCardDialog.ConfirmCallBack() {
          @Override
          public void onOther() {
            AssociationCommonDialog dialog = new AssociationCommonDialog(MainFragment.this.getContext());
            dialog.setContent(getString(R.string.tip), getString(R.string.tip_whether_to_exit_ranking_mode));
            dialog.addBtn(getString(R.string.btn_once_more), false, commonDialog -> {
              commonDialog.dismiss();
              hiddenNoticeTip();
              initWallData();
//              playingProgressView.setProgressValue(0, 0);
//              requestStartPlay();
              playInfo = null;
              isFirst = true;
              isUploading = false;
...
//            nodeInfoList.add(new NodeInfo(NodeConstant.TYPE_EXPONENT, mExponent_denominator, getString(R.string.lbl_match_tab_rank_1), "(" + mExponent_denominator / 1000 + "km)", "0.00", 0.6F));
  ...
  private void showNoticeTip(String textTip, String voiceTip) {
    ...
    if (!TextUtils.isEmpty(voiceTip) && (boolean) SPUtils.get(MainFragment.this.getContext(), "isOpenNotice", true)) {
      SpeechUtils.getInstance(getContext()).speakText(voiceTip);
    }
  }

  private void hiddenNoticeTip() {
    tvNoticeTip.setText(getString(R.string.tip_come_on));
