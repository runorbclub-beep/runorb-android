...
public class BleService extends Service implements Constants, BleListener {
    ...
    // Stop scanning after 10 seconds.  默认扫描两分钟
     ...
     * 订阅通知，一定要写在写入之前，要不然就收不到写入的数据，务必使写入和订阅的service是同一个
     *
    ...
    public boolean setCharacteristicNotification(String serviceUUID, String characteristicUUID,
                                                 boolean enabled) {
        if (mBluetoothAdapter == null || mBluetoothGatt == null) {
            Log.w(TAG, "BluetoothAdapter not initialized");
            return false;
...
//        }
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            mScanCallback = new ScanCallback() {
                ...
                public void onScanResult(int callbackType, ScanResult result) {
                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                        if (mScanLeDeviceList.contains(result.getDevice())){
                            return;
                        }
                        mScanLeDeviceList.add(result.getDevice());
                        if (mOnLeScanListener != null && isWanliqiuName(result.getDevice().getName())) {
                            mOnLeScanListener.onLeScan(result.getDevice(), result.getRssi(), result.getScanRecord().getBytes());
                        }
                        //broadcastUpdate(ACTION_BLUETOOTH_DEVICE, result.getDevice(),result.getScanRecord().getBytes());
                        if(BuildConfig.DEBUG) {
                            Log.i(TAG, "onScanResult: name: " + result.getDevice().getName() +
                                    ", address: " + result.getDevice().getAddress() +
                                    ", rssi: " + result.getRssi() + ", scanRecord: " + result.getScanRecord().getBytes());
                        }
            ...
            mLeScanCallback = new BluetoothAdapter.LeScanCallback() {
                ...
                public void onLeScan(BluetoothDevice device, int rssi, byte[] scanRecord) {
                    ...
                    if (device == null || mScanLeDeviceList.contains(device)){
                        return;
                    }
                    mScanLeDeviceList.add(device);
                    if (mOnLeScanListener != null && isWanliqiuName(device.getName())) {
                        mOnLeScanListener.onLeScan(device, rssi, scanRecord);
                    }
                    //broadcastUpdate(ACTION_BLUETOOTH_DEVICE, device,scanRecord);
                }
            };
        }
    }
    ...
    private final BluetoothGattCallback mGattCallback = new BluetoothGattCallback() {
        ...
        public void onServicesDiscovered(BluetoothGatt gatt, int status) {
            ...
            // New services discovered 发现设备（真正建立连接）
        ...
        public void onCharacteristicRead(BluetoothGatt gatt,
            ...
            // Result of a characteristic read operation 读操作的回调
    ...
    private void broadcastUpdate(final String action, BluetoothDevice device, byte[] scanRecord) {
        final Intent intent = new Intent(action);
        intent.putExtra("name", device.getName());
        intent.putExtra("address", device.getAddress());
