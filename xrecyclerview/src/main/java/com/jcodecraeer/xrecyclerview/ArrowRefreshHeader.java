...
public class ArrowRefreshHeader extends LinearLayout implements BaseRefreshHeader {
    ...
    public  String friendlyTime(long time) {
        ...
        if(ct == 0) {
            return getContext().getResources().getString(R.string.lbl_now);
            //return "刚刚";
        }

        if(ct > 0 && ct < 60) {
            return ct + getContext().getResources().getString(R.string.lbl_second);
        }

        if(ct >= 60 && ct < 3600) {
            return Math.max(ct / 60,1) + getContext().getResources().getString(R.string.lbl_min);
        }
        if(ct >= 3600 && ct < 86400){
            return ct / 3600 + getContext().getResources().getString(R.string.lbl_hour);
        }
        if(ct >= 86400 && ct < 2592000){
            int day = ct / 86400 ;
            return day + getContext().getResources().getString(R.string.lbl_day);
        }
        if(ct >= 2592000 && ct < 31104000) {
            return ct / 2592000 + getContext().getResources().getString(R.string.lbl_month);
        }
        return ct / 31104000 + getContext().getResources().getString(R.string.lbl_year);
    }
